...
# Time-stamp: <Mon 2019-03-11 14:11 juergen>
#
    ...
    for lyr in shp:
        ...
        for f in lyr:
            ...
            if g.GetGeometryType() == 1:  # point
                residual = 2
                xy = (round(g.GetPoint_2D(0)[0], residual), round(
                    g.GetPoint_2D(0)[1], residual))
            ...
            if g.GetGeometryType() == 2:  # linestring
                ...
                u_node = (round(g.GetPoint_2D(0)[0], residual), round(
                    g.GetPoint_2D(0)[1], residual))
                v_node = (round(g.GetPoint_2D(last)[0], residual), round(
                    g.GetPoint_2D(last)[1], residual))
...
def create_network_graph(road_graph, od_graph, con_edges):
    ...
    for edge in con_edges.edges():
        ...
        v = edge[1]
        w = con_edges[u][v]['id']
        graph.add_edge(w, v, name='a_'+str(w), type='Misc',
                       capacity=10000, length=0, speedlimit=500, oneway=9, damage=0)
        graph.add_edge(v, w, name='b_'+str(w), type='Misc',
                       capacity=10000, length=0, speedlimit=500, oneway=0, damage=0)

    # add self loops for internal traveltime
    for node in od_graph.nodes():
        graph.add_edge(node, node, name='i_'+str(node), type='Internal',
                       capacity=1000, length=3750, speedlimit=45, oneway=8, damage=0)
    return graph

...
#        self.resources_constrains = [[],[(0,7)],[(20,29)]]
    ...
    def damaged_objects(self):
        ...
        for edge in self.G.edges():
            if get_edge_attribute(self.G, edge, 'damage') > 0:
                self.damage_dict[edge] = [get_edge_attribute(
                    self.G, edge, 'name'), get_edge_attribute(self.G, edge, 'oneway')]
        pass
    ...
    def get_enhanced_sequence(self):
        ...
        for values in sequence:
            ...
            intervention_duration_per_unit = self.interventions[object_type,
                                                                condition_state, intervention_type, 1]
            resources = int(
                self.interventions[object_type, condition_state, intervention_type, 2])

            if object == 'Road':
                edge_lenght = get_edge_attribute(self.graph, edge, 'length')
                edge_type = get_edge_attribute(self.graph, edge, 'type')
                intervention_duration = max(int(np.round(
                    (edge_lenght*self.object_width[edge_type]*intervention_duration_per_unit / self.working_hours)*self.duration_subdivision)), 1)

            else:
                intervention_duration = max(int(np.round(
                    (intervention_duration_per_unit / self.working_hours)*self.duration_subdivision)), 1)

            sequence_list.append(
                (edge, intervention_duration, resources, intervention_type))
        ...
        sequence_matrix = [
            [None] * t_max for i in range(self.max_available_resources)]
        ...
        for task in sequence_list:
            resources_array = get_resources_array(
                resources_matrix, task[1], task[2])
        ...
        # remove None values
        sequence_reduced = remove_duplicates(
            [x for x in sequence_reduced if x[0] != None])
        ...
        for v in sequence_list:
            for w in sequence_reduced:
                if v[0] == w[0]:
                    sequence_enhanced.append(
                        (v[0], w[1], v[1], v[2], v[3], w[2]))
    ...
    def repair(self):
        ...
        for tasks in sequence:
            ...
            for task in tasks:
                ...
                # TODO: add also a condition state 0 and remove -1
                condition_state = get_edge_attribute(
                    self.graph, edge, 'damage')-1
                ...
                variable_costs_per_unit = self.interventions[object_type,
                                                             condition_state, intervention_type, 4]
                resources_costs_per_unit = self.interventions[object_type,
                                                              condition_state, intervention_type, 5]
                if object == 'Road':
                    edge_lenght = get_edge_attribute(self.graph, edge, 'length')
                    edge_type = get_edge_attribute(self.graph, edge, 'type')

                    variable_costs = variable_costs_per_unit * \
                        edge_lenght * self.object_width[edge_type]
                    resources_costs = resources_costs_per_unit * \
                        resources_needed * duration * self.working_hours
                ...
                self.output_costs.append(
                    [edge, fix_costs, variable_costs, resources_costs])

                capacity_i = get_edge_attribute(self.graph, edge, 'capacity_i')
                capacity_d = get_edge_attribute(self.graph, edge, 'capacity')
                capacity_n = np.round(
                    capacity_d + (capacity_i - capacity_d) * recovery)

                set_edge_attribute(graph, edge, 'capacity', capacity_n)

                if oneway == 0:
                    edge = (edge[1], edge[0])
                    capacity_i = get_edge_attribute(
                        self.graph, edge, 'capacity_i')
                    capacity_d = get_edge_attribute(
